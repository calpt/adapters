...
class TensorBoardCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that sends the logs to `TensorBoard
...
class WandbCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that sends the logs to `Weight and Biases <https://www.wandb.com/>`__.
    """
...
class CometCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that sends the logs to `Comet ML <https://www.comet.ml/site/>`__.
    """
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._log_metrics(
                    logs, step=state.global_step, epoch=state.epoch, framework="adapter_transformers"
                )


class AzureMLCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that sends the logs to `AzureML
...
class MLflowCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that sends the logs to `MLflow <https://www.mlflow.org/>`__.
    """
    ...
    def setup(self, args, state, model):
        ...
        Environment:
            HF_MLFLOW_LOG_ARTIFACTS (:obj:`str`, `optional`):
                ...
                whatever is in :class:`~adapter_transformers.TrainingArguments`'s ``output_dir`` to the local or remote
...
class NeptuneCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that sends the logs to `Neptune <https://neptune.ai>`.
    """
...
class CodeCarbonCallback(TrainerCallback):
    """
    A :class:`~adapter_transformers.TrainerCallback` that tracks the CO2 emission of training.
    """
