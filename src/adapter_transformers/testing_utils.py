...
def LoggingLevel(level):
    """
    This is a context manager to temporarily change adapter_transformers modules logging level to the desired value and have it
...
class TestCasePlus(unittest.TestCase):
    ...
    * ``pathlib`` objects (all fully resolved):
       ...
       - ``src_dir`` - the directory of ``src`` (i.e. where the ``adapter_transformers`` sub-dir resides)
