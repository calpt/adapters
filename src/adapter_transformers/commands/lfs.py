...
path = /path/to/adapter_transformers/.env/bin/python

args = -m debugpy --listen 5678 --wait-for-client /path/to/adapter_transformers/src/adapter_transformers/commands/transformers_cli.py
...
class LfsCommands(BaseTransformersCLICommand):
    ...
    1. $ adapter_transformers-cli lfs-enable-largefiles
    ...
    2. $ adapter_transformers-cli lfs-multipart-upload
...
class LfsEnableCommand:
    ...
    def run(self):
        warnings.warn(
            "Managing repositories through adapter_transformers-cli is deprecated. Please use `huggingface-cli` instead."
        )
        ...
        if not os.path.isdir(local_path):
            ...
            exit(1)
        subprocess.run(
            "git config lfs.customtransfer.multipart.path adapter_transformers-cli".split(), check=True, cwd=local_path
        )
        subprocess.run(
