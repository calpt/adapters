...
class Adafactor(Optimizer):
    ...
    *warmup_init* options. To use a manual (external) learning rate schedule you should set `scale_parameter=False` and
    ...
    Recommended T5 finetuning settings (https://discuss.huggingface.co/t/t5-finetuning-tips/684/3):
        ...
        When using ``lr=None`` with :class:`~adapter_transformers.Trainer` you will most likely need to use :class:`~adapter_transformers.optimization.AdafactorSchedule` scheduler as following::
...
class AdafactorSchedule(LambdaLR):
    """
    Since :class:`~adapter_transformers.optimization.Adafactor` performs its own scheduling, if the training loop relies on a
...
def get_adafactor_schedule(optimizer, initial_lr=0.0):
    """
    Get a proxy schedule for :class:`~adapter_transformers.optimization.Adafactor`
    ...
    Return:
        :class:`~adapter_transformers.optimization.Adafactor` proxy schedule object.
