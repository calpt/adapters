...
CTRL_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~adapter_transformers.CTRLConfig`): Model configuration class with all the parameters of the model.
...
CTRL_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`Numpy array` or :obj:`tf.Tensor` of shape :obj:`(batch_size, input_ids_length)`):
            ...
            Indices can be obtained using :class:`~adapter_transformers.CTRLTokenizer`. See
            :meth:`adapter_transformers.PreTrainedTokenizer.__call__` and :meth:`adapter_transformers.PreTrainedTokenizer.encode` for
            details.
...
class TFCTRLModel(TFCTRLPreTrainedModel):
    ...
    # Copied from adapter_transformers.models.gpt2.modeling_tf_gpt2.TFGPT2Model.serving_output
...
class TFCTRLLMHeadModel(TFCTRLPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    # Copied from adapter_transformers.models.gpt2.modeling_tf_gpt2.TFGPT2LMHeadModel.serving_output
    def serving_output(self, output):
        pkv = tf.convert_to_tensor(output.past_key_values) if self.config.use_cache else None
        hs = tf.convert_to_tensor(output.hidden_states) if self.config.output_hidden_states else None
        attns = tf.convert_to_tensor(output.attentions) if self.config.output_attentions else None
...
@add_start_docstrings(
    ...
    :class:`~adapter_transformers.TFCTRLForSequenceClassification` uses the last token in order to do the classification, as
