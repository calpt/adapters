...
RAG_START_DOCSTRING = r"""
    ...
    The question encoder can be any `autoencoding` model, preferably :class:`~adapter_transformers.DPRQuestionEncoder`, and the
    generator can be any `seq2seq` model, preferably :class:`~adapter_transformers.BartForConditionalGeneration`.
    ...
    the ``generator``. It has been tested with :class:`~adapter_transformers.DPRQuestionEncoder` as the ``question_encoder``
    and :class:`~adapter_transformers.BartForConditionalGeneration` or :class:`~adapter_transformers.T5ForConditionalGeneration` as the
    ``generator``.

    This model inherits from :class:`~adapter_transformers.PreTrainedModel`. Check the superclass documentation for the generic
    ...
    Args:
        config (:class:`~adapter_transformers.RagConfig`):
            ...
            :meth:`~adapter_transformers.PreTrainedModel.from_pretrained` method to load the model weights.
        question_encoder (:class:`adapter_transformers.PreTrainedModel`):
            An encoder model compatible with the faiss index encapsulated by the ``retriever``.
        generator (:class:`adapter_transformers.PreTrainedModel`):
...
RAG_FORWARD_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_outputs (:obj:`tuple(tuple(torch.FloatTensor)`, `optional`)
            ...
            Used by the (:class:`~adapter_transformers.RagModel`) model during decoding.
