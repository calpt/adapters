...
class DPRConfig(PretrainedConfig):
    r"""
    :class:`~adapter_transformers.DPRConfig` is the configuration class to store the configuration of a `DPRModel`.

    This is the configuration class to store the configuration of a :class:`~adapter_transformers.DPRContextEncoder`,
    :class:`~adapter_transformers.DPRQuestionEncoder`, or a :class:`~adapter_transformers.DPRReader`. It is used to instantiate the
    components of the DPR model.

    This class is a subclass of :class:`~adapter_transformers.BertConfig`. Please check the superclass for the documentation of
    all kwargs.

    Args:
        vocab_size (:obj:`int`, `optional`, defaults to 30522):
            Vocabulary size of the DPR model. Defines the different tokens that can be represented by the `inputs_ids`
            passed to the forward method of :class:`~adapter_transformers.BertModel`.
