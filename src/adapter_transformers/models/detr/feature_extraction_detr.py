...
class DetrFeatureExtractor(FeatureExtractionMixin, ImageFeatureExtractionMixin):
    ...
    This feature extractor inherits from :class:`~adapter_transformers.FeatureExtractionMixin` which contains most of the main
        ...
        **kwargs,
        ...
        Args:
            ...
            annotations (:obj:`Dict`, :obj:`List[Dict]`, `optional`):
                ...
                In case :class:`~adapter_transformers.DetrFeatureExtractor` was initialized with :obj:`format =
            ...
            masks_path (:obj:`pathlib.Path`, `optional`):
                ...
                relevant in case :class:`~adapter_transformers.DetrFeatureExtractor` was initialized with :obj:`format =
                "coco_panoptic"`.
        ...
        Returns:
            :class:`~adapter_transformers.BatchFeature`: A :class:`~adapter_transformers.BatchFeature` with the following fields:
    ...
    def post_process(self, outputs, target_sizes):
        """
        Converts the output of :class:`~adapter_transformers.DetrForObjectDetection` into the format expected by the COCO api.
        Only supports PyTorch.

        Args:
            outputs (:class:`~adapter_transformers.DetrObjectDetectionOutput`):
    ...
    def post_process_segmentation(self, results, outputs, orig_target_sizes, max_target_sizes, threshold=0.5):
        """
        Converts the output of :class:`~adapter_transformers.DetrForSegmentation` into actual instance segmentation
        predictions. Only supports PyTorch.

        Args:
            results (:obj:`List[Dict]`):
                Results list obtained by :meth:`~adapter_transformers.DetrFeatureExtractor.post_process`, to which "masks"
                results will be added.
            outputs (:class:`~adapter_transformers.DetrSegmentationOutput`):
                Raw outputs of the model.
    ...
    def post_process_panoptic(self, outputs, processed_sizes, target_sizes=None, is_thing_map=None, threshold=0.85):
        """
        Converts the output of :class:`~adapter_transformers.DetrForSegmentation` into actual panoptic predictions. Only
        supports PyTorch.
